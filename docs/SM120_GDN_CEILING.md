...
- 2026-05-06 — initial write-up after a session that exhausted the
  patches-at-our-layer search space. Server back on baseline v1.2.4
  config, no changes shipped to the source repo.
- 2026-05-06 (later) — added "Open angles" section: NVFP4 path,
  `sm_120a` missing from wheel arch list, TurboQuant. None tested
  yet. Held the upstream comment until NVFP4 is ruled in/out.
- 2026-05-06 (still later) — **NVFP4 tested and works.** Single 5090,
  `Peutlefaire/Qwen3.6-27B-NVFP4`, `--quantization compressed-tensors`.
  Prefill goes from ~1100 tok/s @ 170W (AutoRound INT4) to ~5300 tok/s
  @ 574-584W (NVFP4) on a 47k-token prompt — full TDP utilization,
  mem-BW% climbs from 0% to 35%. Decode from 73 to 92 tok/s. Coherence
  passes all three tiers. Snapshot `snapshots/start_5090_nvfp4.py`
  shipped on port 5002 (baseline AutoRound stays on 5001).
  `sm_120a` is still missing from the wheel arch list (`fp4_gemm`
...
- 2026-05-06 (validation pass) — Validated NVFP4 across 4 axes: long-ctx
  ...
  that benefits both quants), and a 12-problem coding slice (tied 100%
  with AutoRound — slice too small to resolve small differences; see
  caveat in the validation matrix). Promoted `rtx5090_nvfp4` to default
  recommended, demoted AutoRound snapshots to alternate `stable`. NVFP4
  is now the snapshot we ship for new users on RTX 5090.
  Decided to comment on FlashInfer PR #3088 (consumer-Blackwell GDN
  prefill kernel) with the FFN-vs-GDN cost-share insight as supporting
  data for the PR author rather than the original "unfixable" framing —
  pending user review.
