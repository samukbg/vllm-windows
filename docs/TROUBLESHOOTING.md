...
> **First check: which release are you on?** A wave of install-time
> issues that bit early adopters (`stdout=None` in `_poll_running`,
> `start_pp2_ngram` missing, `CUDA_LIB_PATH is not set`, `run_ninja
> FileNotFoundError`, ninja missing from the zip) was fixed across
> v0.1.13 through v0.1.17 in early May 2026. If you're on any v0.1.x
> release and hit a boot error, run `update.bat` (or download the
> latest zip) before debugging further. The current line is v1.2.x.
...
| Tokenizer load fails with "tokenizer_class 'TokenizersBackend' is not recognised" | Lorbus AutoRound's custom class name | **Auto-fixed since v0.1.5**, the launcher patches `tokenizer_config.json` on every boot. Manual recovery (e.g. when running snapshots without the launcher): `python windows_tools\patch_tokenizer.py D:\models\Qwen3.6-27B-int4-AutoRound`. |
...
| Blackwell boot succeeds but `start_speed` / `start_127k` snapshots fail with "Unknown vLLM environment variable: VLLM_ATTENTION_BACKEND" | vLLM 0.20.0 dropped the env-var form; only the `--attention-backend` CLI arg is read. | Harmless warning if you're on one of the Blackwell zip's `rtx5090_nvfp4*` snapshots (none of them set the env var). For the legacy snapshots running on 0.20, the env var is ignored; the backend is still picked from the CLI flag. To silence the warning, drop the `VLLM_ATTENTION_BACKEND` line from your custom snapshot. |
...
| Boot fails with `ValueError: CUDA_LIB_PATH is not set` or `FileNotFoundError [WinError 2]` from `flashinfer/jit/`, traceback passes through `topk_topp_sampler.py` or `flashinfer_sample` | flashinfer's Windows JIT path expects `CUDA_LIB_PATH` plus a working ninja / `cl.exe`. vLLM 0.19 imports flashinfer unconditionally regardless of `--attention-backend`, so missing any of these kills EngineCore at import or `profile_run`. | Both modes auto-fixed since v0.1.15 / v0.1.16: snapshots probe `CUDA_PATH`, `CUDA_HOME`, and the NVIDIA Toolkit dirs to set `CUDA_LIB_PATH`, and they force `VLLM_USE_FLASHINFER_SAMPLER=0` when MSVC or ninja is missing. The launcher zip has shipped ninja itself since v0.1.17. Run `update.bat` if you're on an older release. To recover the flashinfer-sampler boost, install the free Visual Studio 2022 Build Tools (Desktop development with C++ workload). |
...
| **NVFP4 prefill at ~750 tok/s instead of the validated ~5,300 tok/s on a 47 k prompt; SM=100 % / mem-BW≈0 % / power 200–270 W during load (Blackwell only)** | vLLM's torch-inductor AOT compile cache (`~/.cache/vllm/`) was populated under a polluted CUDA env (typically a system CUDA 12.x install on PATH ahead of the bundled cu13 shim), binding compiled graphs to slow FlashInfer fp4_gemm tactics. The bad picks survive: snapshot env fix, FlashInfer cache wipe, full re-autotune. Recovered to ~5,300 tok/s on the actual hardware after a four-cache nuke. | **Auto-prevented in v1.3.2** (`clean_cuda_env()` scrubs `CUDA_*`/`NVCC_*`/`CUDNN_*` and filters NVIDIA-toolkit + conda `Library/bin` out of PATH; `preflight_sm120a_or_die()` hard-exits in 5 s if the env still can't dispatch sm_120a). On older installs or if the cache was already poisoned: `python windows_tools\wipe_caches.py` (moves the four caches to `.bak.<timestamp>` for forensics by default), then re-launch. Cold cache rebuild takes ~11–25 min on first boot. |
| `[preflight ERROR] FlashInfer sm_120a dispatch is unavailable` at the start of an `rtx5090_nvfp4` boot | The new v1.3.2 preflight refused to launch because FlashInfer can't reach the sm_120a fast path under the cleaned env. Usually means an already-loaded cudart in the parent process tree (e.g. running from a conda env that imported torch earlier) survived the env scrub. | Close every terminal, double-click `start.bat` from a fresh shell. If it persists, capture `where cudart64_13.dll`, `echo %CUDA_PATH%`, and `echo %PATH%` for a bug report; if all three look right and the preflight still fails, the bundled wheel may need a flashinfer rebuild against `compute_120f` (track via [`SM120_GDN_CEILING.md`](SM120_GDN_CEILING.md)). |
| `[Autotuner]: Skipped 6 unsupported tactic(s) for fp4_gemm` printed dozens of times during boot | **Normal.** Those 6 are TMA-WS (warp-specialized) kernels gated on `compute_120f`, which the shipped FlashInfer JIT cache doesn't compile against. The autotuner picks from the remaining tactics. Validated 5,300 tok/s prefill on `rtx5090_nvfp4` is achieved without those 6. See [`SM120_GDN_CEILING.md`](SM120_GDN_CEILING.md). | No action. If you eventually want them, the upstream knob is `FLASHINFER_CUDA_ARCH_LIST=12.0f` at flashinfer-jit-cache build time (not a runtime env var on the shipped wheel). |
| Repeated `[AutoTuner]: Tuning fp4_gemm: 0/13 → 13/13` cycles during boot, looks like an infinite loop | **Normal, and you'll see it on every boot.** Each cycle is a different GEMM shape (per batch size, per MTP draft length, per prefill chunk), not the same work repeating. On the `rtx5090_nvfp4` snapshot expect 10-20 min of autotune on every boot: per-shape tactic selection isn't yet cached on sm_120, that's tracked upstream in FlashInfer. After `wipe_caches.py` add the FlashInfer JIT compile on top of that (~11-25 min total). | Wait for `Application startup complete` / `Uvicorn running on http://0.0.0.0:5001`. Do **not** kill mid-autotune; interrupting starts the cycle over on next boot. The shipped `rtx5090_nvfp4` snapshot prints a banner explaining this at launch time. |
| `Tuning fp4_gemm` loops for hours (2-3 h on a single shape) on every boot; FlashInfer's `trtllm_utils.dll` never finishes building | NVIDIA CUDA 13 Toolkit installed at `C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v13.x`. The space in the path breaks FlashInfer's generated `build.ninja`: `cl.exe` sees the include path unquoted and can't find `cublasLt.h`, so `trtllm_utils.dll` fails to compile and autotune restarts every boot. Reported in [issue #18](https://github.com/devnen/qwen3.6-windows-server/issues/18). | **Auto-fixed in v1.3.10**: the launcher creates a junction `C:\cuda13<minor>` pointing at the installed toolkit (no admin needed) and uses the space-free path. Run `update.bat`. |
