...
| Repeated `[AutoTuner]: Tuning fp4_gemm: 0/13 → 13/13` cycles during boot, looks like an infinite loop | **Normal, and you'll see it on every boot.** Each cycle is a different GEMM shape (per batch size, per MTP draft length, per prefill chunk), not the same work repeating. The first-ever boot is ~9 min because FlashInfer JIT-compiles the fp4_gemm kernels and autotunes tactics for every shape. Every subsequent boot still shows the `Tuning fp4_gemm` progress bars but finishes in ~1-2 min: the compiled kernel binaries are cached under `~/.cache/flashinfer/`, only tactic selection re-runs. After `wipe_caches.py` the full ~11-25 min cold rebuild happens again. | Wait for `Application startup complete` / `Uvicorn running on http://0.0.0.0:5001`. Do **not** kill mid-autotune; interrupting starts the cycle over on next boot. The shipped `rtx5090_nvfp4` snapshot prints a banner explaining this at launch time. |
