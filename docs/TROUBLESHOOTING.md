...
| Blackwell boot succeeds but `start_speed` / `start_127k` snapshots fail with "Unknown vLLM environment variable: VLLM_ATTENTION_BACKEND" | vLLM 0.20.0 dropped the env-var form; only the `--attention-backend` CLI arg is read. | Harmless warning if you're on one of the Blackwell zip's `rtx5090*` snapshots (none of them set the env var). For the legacy snapshots running on 0.20, the env var is ignored — backend is still picked from the CLI flag. To silence the warning, drop the `VLLM_ATTENTION_BACKEND` line from your custom snapshot. |
...
| Repeated `[AutoTuner]: Tuning fp4_gemm: 0/13 → 13/13` cycles during boot, looks like an infinite loop | **Normal.** Each cycle is a different GEMM shape (per batch size, per MTP draft length, per prefill chunk), not the same work repeating. Expect 3–8 min on warm cache, ~9 min on cold cache, ~11–25 min after `wipe_caches.py`. | Wait for `Application startup complete` / `Uvicorn running on http://0.0.0.0:5001`. Do **not** kill mid-autotune; interrupting starts the cycle over on next boot. The shipped `rtx5090_nvfp4` snapshot prints a banner explaining this at launch time. |
