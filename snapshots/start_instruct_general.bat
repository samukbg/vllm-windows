...
if not defined PY (
    echo [start_instruct_general.bat] no python interpreter found under %REPO_ROOT%
    pause
    exit /b 1
)
"%PY%" -u "%~dp0start_instruct_general.py" %*
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" (
    echo.
    echo [start_instruct_general.bat] exited with code %RC%
    pause
)
endlocal & exit /b %RC%
