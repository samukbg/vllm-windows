...
if not defined PY (
    echo [start_5090_nvfp4_vision.bat] no python interpreter found under %REPO_ROOT%
    pause
    exit /b 1
)
"%PY%" -u "%~dp0start_5090_nvfp4_vision.py" %*
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" (
    echo.
    echo [start_5090_nvfp4_vision.bat] exited with code %RC%
    pause
)
endlocal & exit /b %RC%
