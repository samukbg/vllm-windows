...
class AsyncLLM(EngineClient):
    ...
    def __init__(
        ...
        self.idle_monitor_task: asyncio.Task | None = None
        self.last_activity_time = time.monotonic()
        self.vram_cleared = False
        self._sleep_when_idle = os.getenv("VLLM_SLEEP_WHEN_IDLE") == "1"
        self._idle_timeout_s = float(os.getenv("VLLM_IDLE_TIMEOUT_S", "15"))

        try:
            # Start tasks eagerly if we are in the asyncio eventloop.
            asyncio.get_running_loop()
            self._run_output_handler()
            if self._sleep_when_idle:
                self._run_idle_monitor()
        except RuntimeError:
            pass
    ...
    def shutdown(self, timeout: float | None = None) -> None:
        ...
        monitor = getattr(self, "idle_monitor_task", None)
        if monitor is not None:
            cancel_task_threadsafe(monitor)
    ...
    async def add_request(
        ...
        if self._sleep_when_idle:
            if await self.is_sleeping():
                logger.info("New request received, waking up...")
                await self.wake_up()
            self._run_idle_monitor()
    ...
    def _run_idle_monitor(self):
        """Background loop: monitors inactivity and sleeps the engine."""

        if self.idle_monitor_task is not None:
            return

        async def idle_monitor():
            logger.info("Idle monitor started (timeout=%ds)", self._idle_timeout_s)
            try:
                while True:
                    await asyncio.sleep(1.0)
                    now = time.monotonic()
                    if not self.output_processor.has_unfinished_requests():
                        if now - self.last_activity_time >= self._idle_timeout_s:
                            if not await self.is_sleeping() and not self.vram_cleared:
                                logger.info(
                                    "Engine idle for %ds, clearing VRAM...",
                                    int(now - self.last_activity_time),
                                )
                                try:
                                    await self.engine_core.collective_rpc_async("clear_vram")
                                    self.vram_cleared = True
                                except Exception:
                                    logger.warning("Failed to clear VRAM.")
                    else:
                ...
                logger.exception("Idle monitor failed.")

        self.idle_monitor_task = asyncio.create_task(
            idle_monitor(), name="EngineIdleMonitorTask"
        )
